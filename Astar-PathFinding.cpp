// Astar-PathFinding.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
private:
    struct sNode {
        ...
        int y;
        std::vector<sNode*> neighbours;     // Connections to neighbours
...
protected:
    virtual bool OnUserCreate() {
        ...
        // Manually position the start and end markers so they are not a nullptr
    ...
    virtual bool OnUserUpdate(float fElapsedTime) {
        ...
        // Draw Connections FIrst - lines from this nodes position to its connected neighbour node position
        Fill(0, 0, ScreenWidth(), ScreenHeight(), L' ');
        for (int x = 0; x < nMapWidth; x++) {
            for (int y = 0; y < nMapHeight; y++) {
                for (auto n : nodes[y * nMapWidth + x].neighbours) {
                    DrawLine(
                        x * nNodeSize + nNodeSize / 2,
                        y * nNodeSize + nNodeSize / 2,
                        n->x * nNodeSize + nNodeSize / 2,
                        n->y * nNodeSize + nNodeSize / 2,
                        PIXEL_SOLID,
                        FG_DARK_BLUE);
                }
            }
        ...
        for (int x = 0; x < nMapWidth; x++) {
            for (int y = 0; y < nMapHeight; y++) {
                //Fill(x * nNodeSize + nNodeBorder, 
                //    y * nNodeSize + nNodeBorder, 
                //    (x + 1) * nNodeSize - nNodeBorder, 
                //    (y + 1) * nNodeSize - nNodeBorder,
                //    PIXEL_SOLID,
                //    FG_BLUE);
                ...
                Fill(
                    ...
                    (y + 1) * nNodeSize - nNodeBorder,
                    PIXEL_HALF,
                    nodes[y * nMapWidth + x].bObstacle ? FG_WHITE : FG_BLUE);
        ...
        if (nodeEnd != nullptr) {
            ...
            while (p->parent != nullptr) {
                DrawLine(
                    p->x * nNodeSize + nNodeSize / 2,
                    p->y * nNodeSize + nNodeSize / 2,
                    p->parent->x * nNodeSize + nNodeSize / 2,
                    p->parent->y * nNodeSize + nNodeSize / 2,
                    PIXEL_SOLID,
                    FG_YELLOW);
